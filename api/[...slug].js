...
export default async function handler(req, res) {
  ...
  if (path === "/catalog/trending") {
    ...
    res.status(200).json(data.results.filter((r) => r.media_type !== "person").map((r) => mapTitle(r, mediaType === "all" ? undefined : mediaType)));
    return;
  }
  ...
  if (path === "/catalog/list") {
    ...
    if (category === "animation") {
      ...
      res.status(200).json(data?.results.map((r) => mapTitle(r, mediaType)) || []);
      return;
    }
  ...
  if (path === "/catalog/search") {
    ...
    res.status(200).json(data?.results.filter((r) => r.media_type === "movie" || r.media_type === "tv").map((r) => mapTitle(r)) || []);
    return;
  }
  ...
  // --- /catalog/title/:mediaType/:id/trailer (frontend calls this) ---
  ...
  if (seasonMatch) {
    ...
    res.status(200).json(data?.episodes?.map((ep) => ({
      id: ep.id,
