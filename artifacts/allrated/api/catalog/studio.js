...
export default async function handler(req, res) {
  ...
  if (!Number.isInteger(companyId) || companyId <= 0) { res.status(400).json({ error: "Invalid companyId" }); return; }
  ...
  try {
    ...
    res.status(502).json({ error: "Studio catalog unavailable", results: [] });
  }
}
