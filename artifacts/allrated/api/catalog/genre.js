...
const GENRES = new Set([28, 35, 27, 10749, 878, 80]);
...
export default async function handler(req, res) {
  ...
  if (!GENRES.has(genreId)) {
    res.status(400).json({ error: "Unsupported genre" });
    return;
  }
  if (!key) {
  ...
  try {
    ...
    const upstream = await fetch(`${TMDB_BASE}/discover/${mediaType}?${params}`, {
      headers: { Accept: "application/json" },
    });
    if (!upstream.ok) {
      res.status(502).json({ error: "TMDB request failed", results: [] });
      return;
    }
    const data = await upstream.json();
    res.status(200).json((data?.results || []).map((item) => mapTitle(item, mediaType)));
  } catch {
    res.status(502).json({ error: "Genre catalog unavailable", results: [] });
  }
}
