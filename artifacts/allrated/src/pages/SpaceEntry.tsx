  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#07070b] text-white" data-testid="page-space-logged-out">
      ...
      <div className="relative z-20 flex items-center justify-end gap-4 px-6 pt-6 md:px-12">
        <Link href="/help" className="inline-flex items-center gap-1.5 text-xs font-medium text-white/65 transition hover:text-white">
          <HelpCircle className="h-3.5 w-3.5" /> Help &amp; Support
        </Link>
        <div className="hidden h-4 w-px bg-white/10 sm:block" />
        <div className="flex items-center gap-3">
          <span className="text-xs font-medium text-white/45">Show Ads</span>
          <button type="button" aria-pressed={showAds} onClick={() => setShowAds(!showAds)} className={`relative h-5 w-10 rounded-full transition-colors ${showAds ? "bg-white" : "bg-white/15"}`}>
            <span className={`absolute top-0.5 h-4 w-4 rounded-full bg-black transition-transform ${showAds ? "translate-x-[22px]" : "translate-x-0.5"}`} />
          </button>
        </div>
      </div>
      ...
      <main className="relative z-10 flex min-h-[calc(100vh-64px)] flex-col items-center justify-center px-6 pb-28 pt-8 md:pb-12">
        <section className="space-reveal w-full max-w-4xl overflow-hidden rounded-[30px] border border-white/[0.08] bg-white/[0.025] shadow-[0_30px_120px_rgba(0,0,0,.42)] backdrop-blur-xl">
          <div className="relative h-[190px] w-full overflow-hidden bg-black md:h-[235px]">
            <div className="absolute inset-x-0 top-0 z-10 h-16 bg-gradient-to-b from-black to-transparent" />
            <div className="absolute inset-x-0 bottom-0 z-10 h-28 bg-gradient-to-t from-[#07070b] via-black/80 to-transparent" />
            <div className="grid h-full grid-cols-3 gap-2 px-2 opacity-50 md:gap-3">
              {posterColumns.map((column, index) => (
                <div key={index} className="relative h-full overflow-hidden">
                  <div className={`flex flex-col gap-2 md:gap-3 ${index === 1 ? "space-marquee-b" : "space-marquee-a"}`}>
                    {[...column, ...column].map((src, i) => <img key={`${src}-${i}`} src={src} alt="" className="aspect-[2/3] w-full shrink-0 rounded-xl object-cover" loading="lazy" />)}
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="space-float relative flex flex-col items-center px-7 py-10 text-center md:px-10 md:py-12">
            <div className="mb-6 flex h-16 w-16 items-center justify-center rounded-[20px] border border-white/10 bg-white/[0.06] shadow-[0_18px_60px_rgba(0,0,0,.4)]">
            ...
            <Link href="/login" className="group mt-7 inline-flex h-12 min-w-[190px] items-center justify-center gap-2 rounded-2xl bg-white px-6 text-sm font-bold text-black shadow-[0_14px_45px_rgba(255,255,255,.08)] transition duration-300 hover:-translate-y-0.5 hover:bg-white/90">
