...
function buildFetcher(name: string): (page: number) => Promise<CatalogPage> {
  ...
  return async (page: number): Promise<CatalogPage> => {
    let url: string;
    if (genreId) {
      url = `${BASE}/api/catalog/genre?mediaType=movie&genreId=${genreId}&page=${page}`;
    } else if (trendingParams) {
  ...
  return (
    ...
    >
      <div className="relative rounded-lg overflow-hidden aspect-[2/3] bg-[#1a1c24] ring-1 ring-white/5 transition-all duration-300 group-hover/card:ring-white/20 group-hover/card:-translate-y-1">
        {title.posterPath ? (
          <img src={title.posterPath} alt={title.title} loading="lazy" className="w-full h-full object-cover" />
        ) : (
          <div className="flex h-full w-full items-center justify-center px-2 text-center text-xs text-white/40">{title.title}</div>
        )}
      </div>
      ...
      <div className="flex items-center flex-wrap mt-0.5 gap-x-1 text-[11px] text-white/40">
        {title.voteAverage > 0 && (
          <>
            <svg width="9" height="9" viewBox="0 0 24 24" fill="currentColor" className="text-white/60 shrink-0"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>
            <span className="text-white/60">{title.voteAverage.toFixed(1)}</span><span>·</span>
          </>
        )}
      ...
      <div className="px-6 lg:px-20">
        {isLoading && (
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 gap-x-3 gap-y-6">
            {Array.from({ length: 21 }).map((_, i) => <div key={i} className="animate-pulse rounded-lg bg-white/5 aspect-[2/3]" />)}
          </div>
        )}
        ...
        {!isLoading && allTitles.length > 0 && (
          <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 xl:grid-cols-7 gap-x-3 gap-y-6">
            ...
            {isFetchingNextPage && Array.from({ length: 7 }).map((_, i) => <div key={`skel-${i}`} className="animate-pulse rounded-lg bg-white/5 aspect-[2/3]" />)}
          </div>
        )}

        {!isLoading && allTitles.length === 0 && !isError && (
          <div className="flex flex-col items-center justify-center py-32 text-white/30">
            <svg width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" className="mb-4"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>
