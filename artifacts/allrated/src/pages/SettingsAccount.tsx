    ...
    <div className="flex items-center justify-between pr-0 lg:pr-8">
      <div className="flex items-center gap-5">
        <div className="text-white/70">
          {device.type === "desktop" ? <Laptop className="w-6 h-6" /> : <Smartphone className="w-6 h-6" />}
        </div>
      ...
      <button disabled={device.isCurrent} className="px-6 py-2.5 rounded-lg bg-[#1a1c22] text-[14px] font-semibold text-white/90 disabled:opacity-50">
        Log Out
      </button>
    </div>
  );
        ...
        <div className="px-6 md:px-12">
          ...
          <div className="flex items-center justify-between pr-0 lg:pr-8 mt-10 lg:mt-0 mb-10">
            <div className="flex flex-col gap-1">
              ...
              <span className="text-[16px] font-semibold text-white/90">{profile?.email || "Not signed in"}</span>
            </div>
            {profile ? (
              <button onClick={handleLogout} className="flex items-center gap-2 rounded-lg bg-white/10 px-5 py-2.5 text-[14px] font-semibold text-white hover:bg-white/15">
                <LogOut className="h-4 w-4" /> Sign out
              </button>
            ) : (
              <button onClick={() => navigate('/login')} className="flex items-center gap-2 rounded-lg bg-white px-5 py-2.5 text-[14px] font-semibold text-black hover:bg-white/90">
          ...
          {otherDevices.length > 0 && (
            <>
              ...
              <div className="flex flex-col gap-8">{otherDevices.map((device) => <DeviceRow key={device.id} device={device} />)}</div>
            </>
          )}
        </div>
      </div>
