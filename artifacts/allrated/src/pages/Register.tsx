        ...
        <section className="auth-reveal w-full max-w-[430px]">
          <div className="mb-7 text-center">
            <div className="relative mx-auto mb-5 flex h-[68px] w-[68px] items-center justify-center rounded-[21px] border border-white/15 bg-white/[0.07] shadow-[0_20px_70px_rgba(0,0,0,.45)] backdrop-blur-xl">
              <img src="/brand/logo.png" alt="Bingr" className="h-11 w-11 object-contain" />
          ...
          <form onSubmit={handleSubmit} className="relative overflow-hidden rounded-[28px] border border-white/10 bg-white/[0.055] p-6 shadow-[0_30px_100px_rgba(0,0,0,.48)] backdrop-blur-2xl sm:p-7">
            ...
            <div className="space-y-4">
              <div>
                <label htmlFor="register-username" className="mb-2.5 block text-xs font-semibold uppercase tracking-[0.12em] text-white/45">Username</label>
                <input id="register-username" required value={username} onChange={(e) => setUsername(e.target.value)} placeholder="Your name" className="h-13 w-full rounded-2xl border border-white/10 bg-black/25 px-4 text-[15px] text-white placeholder:text-white/20 outline-none transition duration-300 focus:border-white/30 focus:bg-white/[0.06] focus:ring-4 focus:ring-white/[0.04]" />
              </div>
              <div>
                <label htmlFor="register-email" className="mb-2.5 block text-xs font-semibold uppercase tracking-[0.12em] text-white/45">Email</label>
                <input id="register-email" type="email" autoComplete="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="you@example.com" className="h-13 w-full rounded-2xl border border-white/10 bg-black/25 px-4 text-[15px] text-white placeholder:text-white/20 outline-none transition duration-300 focus:border-white/30 focus:bg-white/[0.06] focus:ring-4 focus:ring-white/[0.04]" />
              </div>
              <div>
                <label htmlFor="register-password" className="mb-2.5 block text-xs font-semibold uppercase tracking-[0.12em] text-white/45">Password</label>
                <div className="relative">
                  <input id="register-password" type={showPassword ? 'text' : 'password'} autoComplete="new-password" required minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="h-13 w-full rounded-2xl border border-white/10 bg-black/25 px-4 pr-12 text-[15px] text-white placeholder:text-white/20 outline-none transition duration-300 focus:border-white/30 focus:bg-white/[0.06] focus:ring-4 focus:ring-white/[0.04]" />
                  <button type="button" onClick={() => setShowPassword((value) => !value)} className="absolute right-2 top-1/2 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-xl text-white/30 transition hover:bg-white/10 hover:text-white/75" aria-label={showPassword ? 'Hide password' : 'Show password'}>{showPassword ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button>
                </div>
              </div>
              <div>
                <label htmlFor="register-confirm" className="mb-2.5 block text-xs font-semibold uppercase tracking-[0.12em] text-white/45">Confirm password</label>
                <div className="relative">
                  <input id="register-confirm" type={showConfirm ? 'text' : 'password'} autoComplete="new-password" required minLength={6} value={confirm} onChange={(e) => setConfirm(e.target.value)} placeholder="••••••••" className="h-13 w-full rounded-2xl border border-white/10 bg-black/25 px-4 pr-12 text-[15px] text-white placeholder:text-white/20 outline-none transition duration-300 focus:border-white/30 focus:bg-white/[0.06] focus:ring-4 focus:ring-white/[0.04]" />
                  <button type="button" onClick={() => setShowConfirm((value) => !value)} className="absolute right-2 top-1/2 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-xl text-white/30 transition hover:bg-white/10 hover:text-white/75" aria-label={showConfirm ? 'Hide password' : 'Show password'}>{showConfirm ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}</button>
                </div>
              </div>
            </div>
