...
export default function EmbedPlayer() {
  ...
  useEffect(() => {
    ...
    const save = async (position: number, duration: number, completed = false) => {
      ...
      if (!token || !Number.isFinite(position) || position < 0) return;
      const now = Date.now();
      ...
      try {
        ...
        // Playback should never be interrupted by progress persistence failures.
      }
    };

    const onMessage = (event: MessageEvent) => {
      if (!['https://embed.vidrift.in', window.location.origin].includes(event.origin) && event.origin !== 'https://vidsrc2.ru') return;
      const data = event.data;
      if (data?.type === 'PLAYER_EVENT') {
        const payload = data.data || {};
        const position = Number(payload.player_progress);
        const duration = Number(payload.player_duration);
        const status = payload.player_status;
        if (status === 'completed') void save(position || duration || 0, duration, true);
        else if (status === 'playing' || status === 'paused' || status === 'seeked') void save(position, duration);
        return;
      }
      if (data?.type === 'vidrift:progress') {
        void save(Number(data.currentTime), Number(data.duration));
        return;
      }
      if (data?.type === 'vidrift:ended') void save(Number(data.duration || 0), Number(data.duration || 0), true);
    };
  ...
  return (
    <div className="fixed inset-0 z-[200] bg-black text-white">
      ...
      {embedUrl ? <iframe key={embedUrl} src={embedUrl} title={`${titleName || 'Video'} player`} className="absolute inset-0 h-full w-full border-0 bg-black" allow="autoplay; fullscreen; encrypted-media; picture-in-picture" allowFullScreen referrerPolicy="origin" onLoad={() => setLoading(false)} /> : <div className="absolute inset-0 grid place-items-center px-6 text-center"><div><div className="text-sm font-semibold">This source is not available for this episode.</div><div className="mt-2 text-xs text-white/45">Choose another source from the selector above.</div></div></div>}
