...
function ServerIcon({ kind }: { kind: 'logo' | 'us' | 'in' }) {
  ...
  if (kind === 'in') return (
    <span className="relative block h-[14px] w-5 shrink-0 overflow-hidden rounded-[2px]" aria-hidden="true">
      <span className="absolute inset-x-0 top-0 h-1/3 bg-[#ff6820]" /><span className="absolute inset-x-0 top-1/3 h-1/3 bg-white" />
      <span className="absolute inset-x-0 bottom-0 h-1/3 bg-[#046a38]" /><span className="absolute left-1/2 top-1/2 h-[6px] w-[6px] -translate-x-1/2 -translate-y-1/2 rounded-full border border-[#07038d]" />
    </span>
  );
  return (
    <span className="relative block h-[14px] w-5 shrink-0 overflow-hidden rounded-[2px] bg-white" aria-hidden="true">
      <span className="absolute inset-x-0 top-0 h-[1.75px] bg-[#d80027]" /><span className="absolute inset-x-0 top-[3.5px] h-[1.75px] bg-[#d80027]" />
      <span className="absolute inset-x-0 top-[7px] h-[1.75px] bg-[#d80027]" /><span className="absolute inset-x-0 top-[10.5px] h-[1.75px] bg-[#d80027]" />
...
function SettingButton({ selected, children, onClick, subtitle }: { selected: boolean; children: ReactNode; onClick: () => void; subtitle?: string }) {
  return (
    <button type="button" onClick={onClick} className="flex items-start gap-3 px-5 py-2.5 text-left transition-colors hover:bg-white/10">
      <span className="mt-0.5 w-4 shrink-0 text-blue-400" style={{ opacity: selected ? 1 : 0 }}>✓</span>
      <div><div className="font-semibold text-[15px] leading-tight">{children}</div>{subtitle ? <div className="mt-0.5 text-[13px] text-white/50">{subtitle}</div> : null}</div>
    </button>
  );
}
...
export default function BingrWatch() {
  ...
  const selectedSource = useMemo(() => {
    ...
    const ranked = sources.map((source) => ({ source, rank: qualityRank(source.quality) }))
      .filter(({ rank }) => rank > 0 && rank <= quality.cap).sort((a, b) => b.rank - a.rank);
  ...
  useEffect(() => {
    ...
    const onLoadedMetadata = () => {
      ...
      if (!firstSourceRef.current && restoreTimeRef.current > 0 && Number.isFinite(video.duration)) video.currentTime = Math.min(restoreTimeRef.current, Math.max(0, video.duration - 0.5));
    ...
    video.addEventListener('loadedmetadata', onLoadedMetadata); video.addEventListener('play', onPlay); video.addEventListener('pause', onPause);
    video.addEventListener('timeupdate', onTime); video.addEventListener('durationchange', onDuration); video.addEventListener('progress', onProgress);
    video.addEventListener('waiting', onWaiting); video.addEventListener('playing', onPlaying);
    return () => {
      video.removeEventListener('loadedmetadata', onLoadedMetadata); video.removeEventListener('play', onPlay); video.removeEventListener('pause', onPause);
      video.removeEventListener('timeupdate', onTime); video.removeEventListener('durationchange', onDuration); video.removeEventListener('progress', onProgress);
  ...
  useEffect(() => {
    ...
    subtitles.forEach((sub) => {
      ...
      track.kind = 'subtitles'; track.src = sub.url; track.label = sub.label; track.srclang = sub.language.slice(0, 2); video.appendChild(track);
    });
  }, [subtitles]);
  ...
  const seekPointer = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    video.currentTime = Math.max(0, Math.min(duration, ((event.clientX - rect.left) / rect.width) * duration)); resetControls();
  };

  const changeVolume = (value: number) => {
    const video = videoRef.current;
    if (!video) return;
    const next = Math.max(0, Math.min(1, value)); video.volume = next; video.muted = next === 0; setVolume(next); setMuted(next === 0);
  };

  const toggleMute = () => {
    const video = videoRef.current;
    if (!video) return;
    video.muted = !video.muted; setMuted(video.muted); resetControls();
  };

  const toggleFullscreen = async () => {
    if (document.fullscreenElement || (document as any).webkitFullscreenElement) { await document.exitFullscreen?.(); return; }
    const root = rootRef.current;
    if (root?.requestFullscreen) await root.requestFullscreen(); else await (root as any)?.webkitRequestFullscreen?.();
  };

  const selectQuality = (id: (typeof QUALITIES)[number]['id']) => {
    restoreTimeRef.current = videoRef.current?.currentTime || 0; setQualityId(id); setMenu(null); resetControls();
  };

  const selectServer = (id: string) => {
    restoreTimeRef.current = videoRef.current?.currentTime || 0; setServerId(id); setMenu(null); resetControls();
  };

  const selectAudio = (label: string, id: number) => {
    if (selectAudioTrack(id)) { setAudio(label); setMenu(null); resetControls(); }
      ...
      <div className={`absolute inset-0 z-[210] bg-gradient-to-b from-black/65 via-transparent to-black/90 transition-opacity duration-300 ${controls || menu || moreLike ? 'opacity-100' : 'pointer-events-none opacity-0'}`}>
        <div className="absolute inset-x-0 top-0 flex items-start justify-between gap-3 px-4 py-4 md:px-8 md:py-6">
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="max-w-[58vw] pt-1 text-lg font-bold leading-tight tracking-wide text-white md:max-w-none md:text-xl">{titleName}</div>
          </div>
          ...
          <div className="flex shrink-0 items-center gap-1 md:gap-2">
            <div className="relative">
              ...
              {menu === 'quality' ? (
                <div className="absolute right-0 top-full z-[240] pt-2">
                  <div className="flex w-max max-w-[calc(100vw-1rem)] flex-col rounded-xl border border-white/10 bg-black/55 py-2 text-sm text-white shadow-2xl backdrop-blur-2xl">
                    <div className="flex">
                      <div className="flex-1 py-2"><div className="mb-2 px-5 text-[11px] font-bold uppercase tracking-wider text-white/50">Quality</div><div className="flex max-h-[250px] flex-col overflow-y-auto">{QUALITIES.map((q) => <SettingButton key={q.id} selected={qualityId === q.id} onClick={() => selectQuality(q.id)} subtitle={`Up to ${q.cap}p`}>{q.label}</SettingButton>)}</div></div>
                      <div className="flex-1 border-l border-white/10 py-2"><div className="mb-2 px-5 text-[11px] font-bold uppercase tracking-wider text-white/50">Server</div><div className="flex max-h-[250px] flex-col overflow-y-auto">{SERVERS.map((server) => <button type="button" key={server.id} onClick={() => selectServer(server.id)} className="flex items-center gap-3 px-5 py-2.5 text-left transition-colors hover:bg-white/10"><span className="w-4 shrink-0 text-blue-400" style={{ opacity: serverId === server.id ? 1 : 0 }}>✓</span><ServerIcon kind={server.icon} /><div className="flex-1 font-semibold text-[15px]">{server.name}</div></button>)}</div></div>
                    </div>
                    <div className="mx-4 my-1 h-px bg-white/10" /><button type="button" onClick={handleReport} className="flex items-center justify-center gap-2 px-4 py-3 text-xs font-semibold text-white/60 transition-colors hover:text-white"><CircleAlert className="h-4 w-4" />Report an Issue</button>
                  </div>
                </div>
              ) : null}
            </div>
            ...
            <div className="relative">
              ...
              {menu === 'audio' ? (
                <div className="absolute right-0 top-full z-[240] pt-2">
                  <div className="w-[340px] max-w-[calc(100vw-1rem)] rounded-xl border border-white/10 bg-black/55 py-2 text-sm text-white shadow-2xl backdrop-blur-2xl">
                    <div className="flex">
                      <div className="flex-1 py-2"><div className="mb-2 px-5 text-[11px] font-bold uppercase tracking-wider text-white/50">Audio</div><div className="flex max-h-[250px] flex-col overflow-y-auto">
                        {audioTracks.length ? audioTracks.map((track) => <button type="button" key={track.id} onClick={() => selectAudio(track.label, track.id)} className="flex items-center gap-3 px-5 py-2.5 text-left transition-colors hover:bg-white/10"><span className="w-4 text-blue-400" style={{ opacity: audio === track.label ? 1 : 0 }}>✓</span><span className="text-sm">{/hindi|hi/i.test(`${track.label} ${track.language}`) ? '🇮🇳' : '🇺🇸'}</span><div className="flex-1"><div className="font-semibold text-[15px] leading-tight">{track.label}</div></div></button>) : <div className="px-5 py-3 text-[13px] text-white/45">No alternate audio tracks</div>}
                      </div></div>
                      <div className="flex-1 border-l border-white/10 py-2"><div className="mb-2 px-5 text-[11px] font-bold uppercase tracking-wider text-white/50">Subtitles</div><div className="flex max-h-[250px] flex-col overflow-y-auto">
                        <button type="button" onClick={() => { setSubtitle('Off'); setMenu(null); resetControls(); }} className="flex items-center gap-3 px-5 py-2.5 text-left transition-colors hover:bg-white/10"><span className="w-4 text-blue-400" style={{ opacity: subtitle === 'Off' ? 1 : 0 }}>✓</span><div className="font-semibold text-[15px]">Off</div></button>
                        {subtitles.map((sub, index) => <button type="button" key={`${sub.label}-${index}`} onClick={() => { setSubtitle(sub.label); setMenu(null); resetControls(); }} className="flex items-center gap-3 px-5 py-2.5 text-left transition-colors hover:bg-white/10"><span className="w-4 text-blue-400" style={{ opacity: subtitle === sub.label ? 1 : 0 }}>✓</span><div className="font-semibold text-[15px]">{sub.label}</div></button>)}
                      </div></div>
                    </div>
                    <div className="mx-4 my-1 h-px bg-white/10" /><button type="button" onClick={handleReport} className="flex w-full items-center justify-center gap-2 px-4 py-3 text-xs font-semibold text-white/60 transition-colors hover:text-white"><CircleAlert className="h-4 w-4" />Report an Issue</button>
                  </div>
                </div>
              ) : null}
            </div>
        ...
        <div className="absolute inset-x-0 bottom-0 px-4 pb-4 md:px-8 md:pb-6">
          ...
          <div className="relative mb-2 flex items-center gap-2 md:gap-3">
            <div className="relative h-5 flex-1 cursor-pointer touch-none" onPointerDown={seekPointer}><div className="absolute left-0 right-0 top-1/2 h-[6px] -translate-y-1/2 overflow-hidden rounded-full bg-white/25"><div className="absolute inset-y-0 left-0 w-full origin-left rounded-full bg-white/40" style={{ transform: `scaleX(${bufferedProgress / 100})` }} /><div className="absolute inset-y-0 left-0 w-full origin-left rounded-full bg-white" style={{ transform: `scaleX(${progress / 100})` }} /></div><div className="absolute left-0 top-1/2 z-10 h-4 w-4 -translate-x-1/2 -translate-y-1/2 rounded-full bg-white shadow-lg" style={{ left: `${progress}%` }} /></div>
          ...
          <div className="relative flex items-center justify-between">
            <div className="flex items-center gap-2"><button type="button" onClick={() => seek(-10)} className="flex select-none items-center justify-center px-2 py-2 text-white/90 transition-opacity hover:text-white" aria-label="Rewind 10 seconds"><Rewind className="h-5 w-5 -scale-x-100 md:h-6 md:w-6" /><span className="ml-0.5 text-sm font-semibold leading-none md:text-base">10</span></button><button type="button" onClick={togglePlay} className="flex items-center justify-center p-2 text-white" aria-label={playing ? 'Pause' : 'Play'}>{playing ? <Pause className="h-8 w-8 md:h-10 md:w-10" /> : <Play className="h-8 w-8 fill-current md:h-10 md:w-10" />}</button><button type="button" onClick={() => seek(10)} className="flex select-none items-center justify-center px-2 py-2 text-white/90 transition-opacity hover:text-white" aria-label="Forward 10 seconds"><span className="mr-0.5 text-sm font-semibold leading-none md:text-base">10</span><FastForward className="h-5 w-5 md:h-6 md:w-6" /></button>
              <div className="flex items-center" onMouseEnter={() => setShowVolume(true)} onMouseLeave={() => setShowVolume(false)}><button type="button" onClick={toggleMute} className="flex items-center justify-center p-1.5 text-white" aria-label="Volume">{muted || volume === 0 ? <VolumeX className="h-6 w-6 md:h-7 md:w-7" /> : <Volume2 className="h-6 w-6 md:h-7 md:w-7" />}</button><div className={`overflow-hidden transition-all duration-200 ${showVolume ? 'w-24 opacity-100' : 'w-0 opacity-0'}`}><div className="relative mx-2 flex h-5 w-20 items-center"><input min="0" max="100" step="1" type="range" value={Math.round(volume * 100)} onChange={(e) => changeVolume(Number(e.target.value) / 100)} className="absolute inset-0 z-10 h-full w-full cursor-pointer opacity-0" aria-label="Volume level" /><div className="relative h-1 w-full overflow-hidden rounded-full bg-white/25"><div className="absolute inset-y-0 left-0 w-full origin-left rounded-full bg-white" style={{ transform: `scaleX(${volume})` }} /></div><div className="pointer-events-none absolute left-0 top-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full bg-white shadow" style={{ left: `${volume * 100}%` }} /></div></div></div>
            </div>
            <div className="flex items-center"><button type="button" onClick={toggleFullscreen} className="p-2 text-white/90 transition-opacity hover:text-white" aria-label="Fullscreen"><Maximize className="h-6 w-6 md:h-7 md:w-7" /></button></div>
          </div>
        </div>

        {moreLike ? (
          <div className="absolute inset-0 z-[250] flex flex-col overflow-hidden bg-black/85 text-white backdrop-blur-md">
            <div className="flex shrink-0 items-center justify-between px-6 py-5 md:px-10"><div className="flex min-w-0 items-baseline gap-4"><h2 className="text-2xl font-bold tracking-wide md:text-[26px]">More Like This</h2><span className="line-clamp-1 text-sm font-medium text-white/40 md:text-lg">{titleName}</span></div><button type="button" onClick={closeMoreLike} className="rounded-full p-2.5 transition-colors hover:bg-white/10" aria-label="Close"><X className="h-7 w-7" /></button></div>
            <div className="mx-6 h-px shrink-0 bg-white/10 md:mx-10" />
            <div className="flex-1 overflow-y-auto px-6 pb-32 pt-4 md:px-10 md:pt-6"><div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 md:gap-6 lg:grid-cols-5">
              {(((title as any)?.similar?.results || (title as any)?.recommendations?.results || []) as any[]).slice(0, 20).map((item: any) => <button type="button" key={item.id} onClick={() => { setMoreLike(false); navigate(`/title/${item.media_type || mediaType}/${item.id}`); }} className="group flex w-full flex-col gap-2 text-left"><div className="relative aspect-video w-full overflow-hidden rounded-xl border border-white/10 bg-white/5 shadow-[0_4px_20px_rgba(0,0,0,0.3)]"><img alt={item.title || item.name || ''} loading="lazy" src={item.backdrop_path ? `https://image.tmdb.org/t/p/w780${item.backdrop_path}` : item.poster_path ? `https://image.tmdb.org/t/p/w780${item.poster_path}` : ''} className="absolute inset-0 h-full w-full object-cover transition-transform duration-500 ease-out group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent opacity-80" /><div className="absolute bottom-1.5 left-1.5 z-10">{item.vote_average ? <div className="rounded bg-black/60 px-1.5 py-0.5 text-[10px] font-bold text-white backdrop-blur-md sm:text-[11px]">★ {Number(item.vote_average).toFixed(1)}</div> : null}</div></div><div className="flex flex-col px-0.5"><h3 className="truncate text-[12px] font-medium leading-snug text-white/90 sm:text-[14px]">{item.title || item.name}</h3><div className="mt-1 truncate text-[9px] font-medium leading-none text-white/50 sm:text-[11px]">{String(item.release_date || item.first_air_date || '').slice(0, 4)}<span className="mx-1">•</span><span className="uppercase tracking-wider">{String(item.media_type || mediaType)}</span></div></div></button>)}
            </div></div>
          </div>
        ) : null}
      </div>
