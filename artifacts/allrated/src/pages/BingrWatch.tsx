...
export default function BingrWatch() {
  ...
  useEffect(() => {
    ...
    restoreTimeRef.current = videoRef.current?.currentTime || 0; setLoading(true); load(selectedSource.url, selectedSource.type);
  ...
  useEffect(() => {
    ...
    video.addEventListener('timeupdate', onTime); video.addEventListener('durationchange', onDuration); video.addEventListener('progress', onProgress); video.addEventListener('waiting', onWaiting); video.addEventListener('playing', onPlaying);
    return () => {
  ...
  useEffect(() => {
    ...
    subtitles.forEach((sub) => { const track = document.createElement('track'); track.kind = 'subtitles'; track.src = sub.url; track.label = sub.label; track.srclang = sub.language.slice(0, 2); video.appendChild(track); });
  }, [subtitles]);

  useEffect(() => {
    const video = videoRef.current; if (!video) return;
    Array.from(video.textTracks).forEach((track) => { track.mode = 'disabled'; });
    if (subtitle !== 'Off') { const track = Array.from(video.textTracks).find((item) => item.label === subtitle); if (track) track.mode = 'showing'; }
  }, [subtitle, subtitles]);
  ...
  const handleReport = async () => {
    ...
    try { if (navigator.share) await navigator.share({ title: 'Bingr Player Issue', text: report }); else if (navigator.clipboard) { await navigator.clipboard.writeText(report); showNotice('Issue details copied'); } else showNotice('Unable to open report tools'); } catch { /* cancelled */ }
  };
