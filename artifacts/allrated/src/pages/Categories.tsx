...
function PopularGenresRow({ expanded = false }: { expanded?: boolean }) {
  ...
  return (
    <div className="w-full relative px-4 md:px-0 mb-[35px]">
      ...
      {expanded ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {GENRES.map((genre) => <Link key={genre.name} href={`/catalog/movie/${encodeURIComponent(`${genre.name} Movies`)}`}><a className="block"><div className="relative rounded-md overflow-hidden aspect-[16/9] bg-[#16181f]"><img alt={genre.name} className="w-full h-full object-cover" loading="lazy" src={genre.image} /></div></a></Link>)}
        </div>
      ) : (
        <div className="relative group">
          <div ref={scrollRef} className="flex overflow-x-auto gap-3 py-4 [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden scroll-smooth snap-x">
            {GENRES.map((genre) => (
              <div key={genre.name} className="snap-start"><Link href={`/catalog/movie/${encodeURIComponent(`${genre.name} Movies`)}`}><a className="block"><div className="flex-none cursor-pointer group/genre relative rounded-md overflow-hidden transition-all duration-300 transform hover:scale-105 hover:z-10 w-[160px] md:w-[220px] lg:w-[280px] aspect-[16/9]"><img alt={genre.name} className="w-full h-full object-cover transition-transform duration-300 group-hover/genre:scale-105" loading="lazy" src={genre.image} /><div className="absolute inset-0 bg-black/0 group-hover/genre:bg-white/5 transition-colors duration-300" /></div></a></Link></div>
            ))}
          </div>
