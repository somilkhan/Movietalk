...
export function useHlsPlayer(videoRef: RefObject<HTMLVideoElement | null>, onFatalErrorRef: RefObject<(() => void) | null>) {
  ...
  const load = useCallback((src: string, type: string) => {
    ...
    if (isHls && Hls.isSupported()) {
      const hls = new Hls({
        fetchSetup(context, init) { return new Request(PROXY + encodeURIComponent(context.url), init); },
        maxBufferLength: 12,
      ...
      hls.on(Hls.Events.ERROR, (_, data) => {
        if (data.fatal) { hls.destroy(); hlsRef.current = null; setAudioTracks([]); onFatalErrorRef.current?.(); }
      });
      hls.loadSource(src);
  ...
  const selectAudioTrack = useCallback((trackId: number) => {
    if (hlsRef.current && trackId >= 0 && trackId < hlsRef.current.audioTracks.length) { hlsRef.current.audioTrack = trackId; return true; }
    return false;
  }, []);
