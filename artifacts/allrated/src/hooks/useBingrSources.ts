...
interface BingrRawSource { url: string; quality?: string | number; language?: string; type?: string; label?: string; name?: string; headers?: Record<string, string>; provider?: { id?: string; name?: string } }
...
function normalizeSources(rawSources: BingrRawSource[], serverId: string, providerName: string) {
  return rawSources.map((s) => {
    ...
    return { url: s.url, type, quality, provider: { id: s.provider?.id ?? serverId, name: s.provider?.name ?? providerName } };
  }).filter((s): s is BingrSource => s !== null);
}

function normalizeSubtitles(rawSubtitles: BingrRawSubtitle[]) {
  return rawSubtitles.map((s) => ({ url: s.url, label: s.label ?? s.name ?? s.language ?? 'Auto', language: s.language ?? s.label ?? 'unknown' })).filter((s) => s.url);
}
  ...
  const fetch_ = useCallback(async () => {
    ...
    try {
      ...
      if (serverId === 'cinepro') {
        ...
        data = (await response.json()) as BingrStreamResponse;
      } else {
      ...
      const parsedSources = normalizeSources(Array.isArray(data.sources) ? data.sources : [], serverId, serverId === 'cinepro' ? 'CinePro' : serverId.toUpperCase());
