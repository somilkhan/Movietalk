...
export interface BingrAudioTrack { label: string; language: string; }
export interface BingrSource { url: string; type: 'hls' | 'mp4' | 'dash'; quality: string; provider: { id: string; name: string }; audioTracks?: BingrAudioTrack[]; }
export interface BingrSubtitle { url: string; label: string; language: string; }
interface BingrRawSource { url: string; quality?: string | number; language?: string; type?: string; label?: string; name?: string; headers?: Record<string, string>; audioTracks?: BingrAudioTrack[]; }
  ...
  const fetch_ = useCallback(async () => {
    ...
    try {
      ...
      const parsedSources: BingrSource[] = rawSources.map((s) => {
        ...
        const rawType = (s.type ?? '').toLowerCase(), urlLower = s.url.toLowerCase();
        let type: BingrSource['type'] = 'mp4';
        if (rawType.includes('mpegurl') || urlLower.includes('.m3u8') || rawType.includes('hls')) type = 'hls';
        else if (rawType.includes('mpd') || urlLower.includes('.mpd')) type = 'dash';
        let quality = 'Auto';
        ...
        return { url: s.url, type, quality, provider: { id: serverId, name: serverId.toUpperCase() }, audioTracks: s.audioTracks };
