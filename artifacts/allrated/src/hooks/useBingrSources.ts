...
export function useBingrSources(
  serverId: string,
  mediaType: 'movie' | 'tv',
  tmdbId: number,
  title: string,
      ...
      const parsedSources: BingrSource[] = rawSources
        .map((s) => {
          ...
          return {
            ...
            type,
            quality,
            provider: { id: serverId, name: `${serverId.toUpperCase()}` },
          };
        })
        .filter((s): s is BingrSource => s !== null);

      const parsedSubtitles: BingrSubtitle[] = rawSubtitles
        .map((s) => ({
          url: s.url,
          label: s.label ?? s.name ?? s.language ?? 'Auto',
