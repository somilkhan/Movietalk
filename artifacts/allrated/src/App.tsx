...
function PageLoader() { return <div className="min-h-screen bg-black flex items-center justify-center" role="status" aria-label="Loading"><div className="h-10 w-10 rounded-full border-[3px] border-white/10 border-t-white animate-spin" /></div>; }
function LegacyCatalogRedirect() { const { name } = useParams<{ name: string }>(); return <Redirect to={`/catalog/movie/${encodeURIComponent(decodeURIComponent(name ?? ''))}`} />; }
function DetailRedirect({ mediaType }: { mediaType: 'movie' | 'tv' | 'anime' }) { const { id } = useParams<{ id: string }>(); return <Redirect to={`/title/${mediaType}/${id}`} />; }
...
function AppRouter() {
  ...
  return <><ScrollToTop /><GlobalUiFixes /><Switch>
    ...
    <Route path="/search"><Suspense fallback={<PageLoader />}><SearchPage /></Suspense></Route>
    ...
    <Route path="/movie/:id"><DetailRedirect mediaType="movie" /></Route><Route path="/tv/:id"><DetailRedirect mediaType="tv" /></Route><Route path="/anime/:id"><DetailRedirect mediaType="anime" /></Route><Route path="/title/:mediaType/:id"><Suspense fallback={<PageLoader />}><TitleDetail /></Suspense></Route>
    <Route path="/category/:name"><LegacyCatalogRedirect /></Route><Route path="/genre/:name"><LegacyCatalogRedirect /></Route><Route path="/language/:name"><LegacyCatalogRedirect /></Route><Route path="/studio/:name"><LegacyCatalogRedirect /></Route>
    <Route path="/category"><Redirect to="/categories" /></Route><Route path="/genre"><Redirect to="/categories" /></Route><Route path="/language"><Redirect to="/categories" /></Route><Route path="/categories/all/:type"><Redirect to="/categories" /></Route>
    ...
    <Route path="/browse"><Redirect to="/explore" /></Route><Route path="/discover"><Redirect to="/explore" /></Route>
...
export default function App() { return <ErrorBoundary><QueryClientProvider client={queryClient}><WouterRouter><TooltipProvider><NetworkStatus /><ProfileGuard><AppRouter /></ProfileGuard><Toaster /><SonnerToaster position="bottom-center" toastOptions={{ style: { background: '#08080b', color: '#fff', border: '1px solid rgba(255,255,255,.08)' } }} /></TooltipProvider></WouterRouter></QueryClientProvider></ErrorBoundary>; }
