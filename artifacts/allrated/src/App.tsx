...
function PageLoader() {
  return <div className="min-h-screen bg-black flex items-center justify-center" role="status" aria-label="Loading"><div className="h-10 w-10 rounded-full border-[3px] border-white/10 border-t-white animate-spin" /></div>;
}
function LegacyCatalogRedirect() {
  const { name } = useParams<{ name: string }>();
  return <Redirect to={`/catalog/movie/${encodeURIComponent(decodeURIComponent(name ?? ""))}`} />;
}
function DetailRedirect({ mediaType }: { mediaType: "movie" | "tv" | "anime" }) {
...
function AppRouter() {
  ...
  return <>
    ...
    <Switch>
      ...
      <Route path="/movie/:id"><DetailRedirect mediaType="movie" /></Route>
      <Route path="/tv/:id"><DetailRedirect mediaType="tv" /></Route>
      <Route path="/anime/:id"><DetailRedirect mediaType="anime" /></Route>
