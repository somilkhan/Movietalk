...
const item = 'flex w-full items-center gap-2.5 rounded-lg px-2.5 py-2.5 text-left transition-colors hover:bg-white/[0.07] focus-visible:bg-white/[0.08] focus-visible:outline-none';
        ...
        <div className="max-h-[300px] overflow-y-auto pr-0.5">
          {backend === 'movietalk' ? (
            movietalkSources.length === 0 ? <div className="px-3 py-4 text-xs font-medium text-white/35">No sources available</div> : movietalkSources.map((source, idx) => {
              ...
              return <button type="button" role="menuitem" aria-current={active ? 'true' : undefined} key={`${source.provider.id}-${idx}`} onClick={() => { onMovietalkSelect(idx); close(); }} className={cn(item, active && 'bg-white/[0.055]')}>
