  ...
  return (
    <div className="relative" onMouseLeave={() => setOpen(false)}>
      <button
        ...
        className="group flex h-9 items-center gap-2 rounded-lg border border-white/[0.07] bg-black/45 px-2.5 text-white/75 shadow-lg backdrop-blur-xl transition hover:border-white/[0.12] hover:bg-white/[0.08] hover:text-white md:h-10 md:px-3"
      >
      ...
      <div className={cn('absolute bottom-full right-0 z-50 mb-2 origin-bottom-right transition-all duration-150', open ? 'scale-100 opacity-100' : 'pointer-events-none scale-95 opacity-0')}>
        <div role="menu" className="min-w-[170px] overflow-hidden rounded-xl border border-white/[0.08] bg-[#09090b]/95 p-1.5 text-white shadow-[0_18px_50px_-12px_rgba(0,0,0,.85)] backdrop-blur-2xl">
          {options.map((option) => (
            <button
              type="button"
              role="menuitem"
              ...
              className={cn('flex w-full items-center gap-2.5 rounded-lg px-3 py-2.5 text-left text-[12px] transition-colors hover:bg-white/[0.07]', backend === option.value ? 'bg-white/[0.06] font-semibold text-white' : 'font-medium text-white/55')}
            >
