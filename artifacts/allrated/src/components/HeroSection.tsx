    ...
    <section className="relative w-full h-[75vh] md:h-auto md:aspect-video max-h-[85vh] overflow-hidden group" data-testid="hero-section">
      {trailerUrl && !backdropError ? (
        ...
        <div className="absolute inset-0 z-0 bg-black overflow-hidden pointer-events-none">
          <iframe src={youtubeEmbed} title={`${title.title} trailer`} className="absolute left-1/2 top-1/2 w-[177.78vh] min-w-full h-[56.25vw] min-h-full -translate-x-1/2 -translate-y-1/2 scale-[1.35] opacity-90" allow="autoplay; encrypted-media; picture-in-picture" referrerPolicy="strict-origin-when-cross-origin" onError={() => setBackdropError(true)} />
        </div>
      ) : (
        <>
          {backdropUrl && !backdropError && <img src={backdropUrl} alt={title.title} className="absolute inset-0 h-full w-full object-cover z-0" onError={() => setBackdropError(true)} />}
          {(!backdropUrl || backdropError) && <div className="absolute inset-0 bg-black z-0" />}
        </>
      )}
      ...
      <div className="absolute top-5 left-5 z-20 md:hidden">
        <svg width="28" height="28" viewBox="0 0 24 24" fill="white" className="drop-shadow-lg"><path d="M12 0L14.59 9.41L24 12L14.59 14.59L12 24L9.41 14.59L0 12L9.41 9.41L12 0Z"/></svg>
      </div>
      ...
      <div className="absolute inset-0 flex flex-col justify-end px-6 pb-10 md:pb-16 md:pl-[100px] lg:pl-[120px] pointer-events-none z-10">
        <div className="flex flex-col gap-3 max-w-2xl">
          ...
          <div className="flex items-center gap-3 mt-2 pointer-events-auto">
            <Link href={`/watch/${title.mediaType}/${title.id}`} className="flex items-center justify-center w-14 h-14 rounded-full bg-white text-black hover:bg-white/90 transition-transform hover:scale-105 active:scale-95" aria-label={`Play ${title.title}`}><svg width="24" height="24" viewBox="0 0 24 24" fill="currentColor" className="ml-1"><path d="M8 5v14l11-7z"/></svg></Link>
