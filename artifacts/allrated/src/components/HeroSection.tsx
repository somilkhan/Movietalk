...
export function HeroSection({ titles }: { titles: Title[] | undefined }) {
  ...
  if (!title) return <div className="relative aspect-[10/16] min-h-[80svh] w-full overflow-hidden bg-black md:aspect-video md:h-auto md:min-h-0 md:max-h-[85vh]" />;
  ...
  return (
    <section className="relative aspect-[10/16] min-h-[80svh] w-full overflow-hidden bg-black md:aspect-video md:h-auto md:min-h-0 md:max-h-[85vh]" data-testid="hero-section">
      {trailerUrl ? (
      ...
      <div className="absolute left-[clamp(24px,5.8vw,48px)] top-[clamp(24px,5.8vw,48px)] z-20 md:hidden">
        <svg width="28" height="28" viewBox="0 0 24 24" fill="white" aria-hidden="true"><path d="M12 0L14.59 9.41L24 12L14.59 14.59L12 24L9.41 14.59L0 12L9.41 9.41L12 0Z" /></svg>
      </div>

      <div className="absolute inset-x-0 bottom-0 z-10 px-[clamp(24px,5.8vw,48px)] pb-[clamp(80px,18vw,160px)] pt-[clamp(80px,16vw,140px)]">
        <div className="relative max-w-2xl">
          <div className="flex max-w-2xl flex-col gap-[clamp(10px,1.8vw,14px)]">
            {logoPath ? <img src={logoPath} alt={title.title} className="h-auto max-h-[clamp(64px,14vw,100px)] w-auto max-w-[82vw] object-contain object-left drop-shadow-2xl" onError={() => setLogoPath(null)} /> : <h1 className="text-[clamp(2rem,6vw,3.75rem)] font-bold leading-[1.02] tracking-[-0.025em] text-white drop-shadow-lg">{title.title}</h1>}
            <div className="flex items-center gap-[clamp(7px,1.3vw,10px)] text-[clamp(14px,2.4vw,17px)] text-white/90">
            ...
            {title.overview ? <p className="line-clamp-3 max-w-xl text-[clamp(15px,2.5vw,17px)] leading-[1.7] text-white/75">{title.overview}</p> : null}
            <div className="mt-[clamp(2px,0.8vw,6px)] flex items-center gap-[clamp(14px,3vw,22px)]">
              {hasTrailer ? <button type="button" onClick={togglePlay} aria-label={isPlaying ? 'Pause trailer' : 'Play trailer'} className="flex h-[clamp(64px,11.5vw,80px)] w-[clamp(64px,11.5vw,80px)] shrink-0 items-center justify-center rounded-full bg-white text-black transition-transform hover:scale-105 active:scale-95">{isPlaying ? <svg width="19" height="19" viewBox="0 0 24 24" fill="currentColor"><path d="M7 5h4v14H7zM13 5h4v14h-4z" /></svg> : <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z" /></svg>}</button> : null}
              <Link href={`/title/${title.mediaType}/${title.id}`} className="flex min-w-[clamp(180px,33vw,230px)] items-center justify-center gap-2 rounded-full border border-white/15 bg-white/10 px-[clamp(20px,4.5vw,32px)] py-[clamp(14px,2.5vw,18px)] text-[clamp(15px,2.5vw,18px)] font-semibold text-white backdrop-blur-md transition-all hover:bg-white/20 hover:scale-[1.02] active:scale-95">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10" /><path d="M12 16v-4" /><path d="M12 8h.01" /></svg>See More
              </Link>
            </div>
          </div>
          {hasTrailer && isPlaying ? <button type="button" onClick={toggleMute} aria-label={isMuted ? 'Unmute trailer' : 'Mute trailer'} className="absolute bottom-0 right-0 flex h-[clamp(56px,9vw,64px)] w-[clamp(56px,9vw,64px)] items-center justify-center rounded-full border border-white/10 bg-white/10 text-white backdrop-blur-md transition-transform hover:scale-105 active:scale-95 md:right-4">{isMuted ? <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 5 6 9H2v6h4l5 4z" /><path d="m19 9-4 6" /><path d="m15 9 4 6" /></svg> : <svg width="19" height="19" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M11 5 6 9H2v6h4l5 4z" /><path d="M19 9a5 5 0 0 1 0 6" /><path d="M15 12h.01" /></svg>}</button> : null}
        </div>
      </div>
