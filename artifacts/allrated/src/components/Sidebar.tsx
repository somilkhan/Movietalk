...
const NAV_ITEMS = [
  {
    ...
    badge: 0,
    svg: (
      <svg width="22px" height="22px" strokeWidth="1.5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" color="currentColor">
  ...
  },
  {
    ...
    badge: 0,
    svg: (
      <svg width="22px" height="22px" viewBox="0 0 24 24" strokeWidth="1.5" fill="none" xmlns="http://www.w3.org/2000/svg" color="currentColor">
  ...
  },
  {
    ...
    badge: 0,
    svg: (
      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 192 192" fill="none" width="22px" height="22px">
  ...
  },
  {
    ...
    badge: 0,
    svg: (
      <svg width="22px" height="22px" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
  ...
  },
  {
    ...
    badge: 0,
    svg: (
      <svg width="22px" height="22px" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
...
export function Sidebar() {
  ...
  return (
    <aside
      className="fixed left-0 top-0 bottom-0 z-40 w-[60px] bg-[#07070b] flex-col hidden md:flex"
    ...
    >
      ...
      <div className="flex items-center justify-center h-16 flex-shrink-0">
        <Link href="/home" data-testid="link-logo">
          <AnimatedCatLogo size={32} autoPlay />
        </Link>
      </div>

      {/* Nav items */}
      <nav className="flex-1 flex flex-col py-2 items-center gap-1 overflow-y-auto no-scrollbar">
        {NAV_ITEMS.map((item) => {
          const isActive =
            location === item.href ||
            ...
            >
              <button
                className={cn(
                  ...
                  isActive
                    ? "text-white"
                    : "text-[#ffffff66] hover:text-white hover:bg-white/5"
                )}
              ...
              >
                ...
                {isActive && (
                  <span className="absolute left-[-10px] top-1/2 -translate-y-1/2 w-[3px] h-5 bg-[#4752c4] rounded-r-full" />
                )}
                ...
                {/* Badge */}
                {item.badge > 0 && (
                  <span className="absolute -top-1 -right-1 min-w-[16px] h-4 px-0.5 bg-[#4752c4] text-white text-[9px] font-bold rounded-full flex items-center justify-center">
                    {item.badge > 99 ? '99+' : item.badge}
                  </span>
                )}

                {/* Tooltip */}
                <div className="absolute left-full ml-3 px-2 py-1 bg-[#252830] text-white text-xs rounded-md whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none border border-[#ffffff1a] z-50">
                  {item.label}
                </div>
              </button>
            </Link>
