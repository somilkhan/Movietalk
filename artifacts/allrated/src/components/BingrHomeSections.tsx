...
function RankedCard({ item, rank }: { item: Title; rank: number }) {
  return (
    <Link href={routeOf(item)} className="flex-shrink-0 group/card relative flex items-center pr-2 lg:pr-6">
      <div aria-hidden="true" className="select-none z-10 pl-2 lg:pl-4 text-[100px] md:text-[120px] lg:text-[140px] font-normal leading-none tracking-[-0.05em] mr-[-10px] transform scale-x-[1.2] origin-left bg-gradient-to-r from-white via-white to-transparent bg-clip-text text-transparent" style={{ fontFamily: "'Alfa Slab One', 'Arial Black', Impact, sans-serif" }}>{rank}</div>
      <div className="relative flex flex-col w-[110px] sm:w-[130px] lg:w-[160px] z-20 shrink-0">
        <div className="relative rounded-lg overflow-hidden aspect-[2/3] bg-[#1a1c24] ring-1 ring-white/5 transition-all duration-300 group-hover/card:ring-white/20 group-hover/card:-translate-y-2"><img alt={item.title} className="w-full h-full object-cover" loading="lazy" src={poster(item.posterPath)} /></div>
      </div>
    </Link>
  );
}
  ...
  return (
    <main className="relative min-h-screen bg-black text-white pb-20" data-testid="bingr-home">
      ...
      <div className="relative z-10 -mt-4">
        <Tray title="Trending Right Now" items={trending} href="/catalog/all/Trending%20Right%20Now" ranked />
        <Tray title="New Movies" items={newMovies} href="/catalog/movie/New%20Movies" />
        <Tray title="Popular Movies" items={popularMovies} href="/catalog/movie/Popular%20Movies" />
        <Tray title="Popular TV Shows" items={popularTv} href="/catalog/tv/Popular%20TV%20Shows" />
        <Tray title="Top Rated TV Shows" items={topRatedTv} href="/catalog/tv/Top%20Rated%20TV%20Shows" ranked />
        <Tray title="Top Rated Movies" items={topRatedMovies} href="/catalog/movie/Top%20Rated%20Movies" />
        <Tray title="Top Rated Anime" items={topRatedAnime} href="/anime" />
