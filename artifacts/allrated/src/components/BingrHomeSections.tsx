...
function Tray({ title, items, href, ranked = false }: TrayProps) {
  ...
  return <section className="px-6 lg:px-20 pt-8"><Heading title={title} href={href} /><div className="relative group/row"><div ref={scrollRef} className="no-scrollbar flex gap-3 overflow-x-auto scroll-smooth pt-4">{items.map((item, index) => ranked ? <RankedCard key={`${item.mediaType}-${item.id}`} item={item} rank={index + 1} /> : <PosterCard key={`${item.mediaType}-${item.id}`} item={item} />)}</div><button type="button" onClick={next} className="absolute right-0 top-0 bottom-0 z-30 w-10 bg-gradient-to-l from-black to-transparent flex items-center justify-center opacity-0 group-hover/row:opacity-100 transition" aria-label={`Next ${title}`}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round" aria-hidden="true"><path d="M9 18l6-6-6-6" /></svg></button></div></section>;
}
