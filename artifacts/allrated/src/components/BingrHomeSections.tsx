...
function Tray({ title, items, href, ranked = false }: TrayProps) {
  ...
  return <section className="px-6 lg:px-20 pt-8"><Heading title={title} href={href} /><div className="relative group/row"><div ref={scrollRef} className="flex gap-3 overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden scroll-smooth pt-4">{items.map((item, index) => ranked ? <RankedCard key={`${item.mediaType}-${item.id}`} item={item} rank={index + 1} /> : <PosterCard key={`${item.mediaType}-${item.id}`} item={item} />)}</div><button type="button" onClick={next} className="absolute right-0 top-0 bottom-0 z-30 w-10 bg-gradient-to-l from-black to-transparent flex items-center justify-center opacity-0 group-hover/row:opacity-100 transition" aria-label={`Next ${title}`}><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round" aria-hidden="true"><path d="M9 18l6-6-6-6" /></svg></button></div></section>;
}

export interface BingrHomeSectionsProps { trending: Title[]; popularMovies: Title[]; popularTv: Title[]; topRatedMovies: Title[]; topRatedTv: Title[]; topRatedAnime: Title[]; genreRows: Array<{ title: string; items: Title[]; href: string }>; children: ReactNode; }
export function BingrHomeSections({ trending, popularMovies, popularTv, topRatedMovies, topRatedTv, topRatedAnime, genreRows, children }: BingrHomeSectionsProps) {
  return <main className="relative min-h-screen bg-black text-white pb-20" data-testid="bingr-home">{children}<div className="relative z-10 -mt-4"><Tray title="Trending Right Now" items={trending} href="/category/Trending%20Right%20Now" /><Tray title="Popular Movies" items={popularMovies} href="/category/Popular%20Movies" /><Tray title="Popular TV Shows" items={popularTv} href="/category/Popular%20TV%20Shows" /><Tray title="Top Rated TV Shows" items={topRatedTv} href="/category/Top%20Rated%20TV%20Shows" ranked /><Tray title="Top Rated Movies" items={topRatedMovies} href="/category/Top%20Rated%20Movies" /><Tray title="Top Rated Anime" items={topRatedAnime} href="/category/Top%20Rated%20Anime" />{genreRows.map((row) => <Tray key={row.title} title={row.title} items={row.items} href={row.href} />)}</div></main>;
}
