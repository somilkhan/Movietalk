...
const poster = (path?: string | null) =>
  !path ? "/placeholder-poster.svg" : /^https?:\/\//i.test(path) ? path : `https://image.tmdb.org/t/p/w500${path}`;
...
const routeOf = (item: Title) => `/${item.mediaType === "tv" ? "tv" : item.mediaType === "movie" ? "movie" : "anime"}/${item.id}`;
...
function PosterCard({ item }: { item: Title }) {
  return (
    <Link href={routeOf(item)} className="flex-shrink-0 group/card relative w-[130px] md:w-[160px] lg:w-[185px]">
        ...
        <div className="flex items-center mt-1 text-[11px] font-medium text-white/50">
          {item.voteAverage > 0 ? (
            <span className="flex items-center">
              <svg width="10" height="10" viewBox="0 0 24 24" fill="white" className="mr-1" aria-hidden="true">
...
function RankedCard({ item, rank }: { item: Title; rank: number }) {
  return (
    <Link href={routeOf(item)} className="flex-shrink-0 group/card relative flex items-center pr-2 lg:pr-6">
      <div
        aria-hidden="true"
        className="select-none z-10 pl-2 lg:pl-4 text-[100px] md:text-[120px] lg:text-[140px] font-normal leading-none tracking-[-0.05em] mr-[-10px] transform scale-x-[1.2] origin-left bg-gradient-to-r from-white via-white to-transparent bg-clip-text text-transparent"
        style={{ fontFamily: "'Alfa Slab One', 'Arial Black', Impact, sans-serif" }}
      >
        {rank}
      </div>
...
function Tray({ title, items, href, ranked = false }: TrayProps) {
  ...
  const next = () => {
    ...
    if (node) node.scrollBy({ left: node.clientWidth * 0.75, behavior: "smooth" });
  };
  ...
  return (
    <section className="px-6 lg:px-20 pt-8">
      ...
      <div className="relative group/row">
        <div
          ref={scrollRef}
          className="flex gap-3 overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden scroll-smooth pt-4"
        >
          {items.map((item, index) => ranked
            ? <RankedCard key={`${item.mediaType}-${item.id}`} item={item} rank={index + 1} />
            : <PosterCard key={`${item.mediaType}-${item.id}`} item={item} />)}
        </div>
        <button
          type="button"
          onClick={next}
          className="absolute right-0 top-0 bottom-0 z-30 w-10 bg-gradient-to-l from-black to-transparent flex items-center justify-center opacity-0 group-hover/row:opacity-100 transition"
...
export function BingrHomeSections({
  ...
  genreRows,
  children,
}: BingrHomeSectionsProps) {
  return (
    <main className="relative min-h-screen bg-black text-white pb-20" data-testid="bingr-home">
      {children}
      <div className="relative z-10 -mt-4">
        <Tray title="Trending Right Now" items={trending} href="/category/Trending%20Right%20Now" ranked />
        <Tray title="New Movies" items={newMovies} href="/category/New%20Movies" />
