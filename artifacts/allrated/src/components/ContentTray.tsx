...
export function ContentTray({
  ...
  const scroll = (direction: 'left' | 'right') => {
    const el = scrollRef.current;
    if (!el) return;
    const scrollAmount = direction === 'left' ? -el.clientWidth * 0.72 : el.clientWidth * 0.72;
  ...
  return (
    <section className={cn('relative mb-8 md:mb-10', className)} aria-labelledby={`tray-${heading.replace(/\W+/g, '-').toLowerCase()}`}>
      <div className="flex items-center justify-between mb-4 px-4 sm:px-6 lg:px-20">
        <h2 id={`tray-${heading.replace(/\W+/g, '-').toLowerCase()}`} className="text-[17px] lg:text-[19px] font-semibold tracking-tight text-white/90">
          {heading}
        </h2>
        {viewAllHref && (
          <Link
            href={viewAllHref}
            className="group flex min-h-9 items-center gap-1.5 rounded-full bg-white/5 px-3 py-1.5 text-[12px] font-semibold text-white/55 transition-colors hover:bg-white/10 hover:text-white focus-visible:outline-none"
          >
            View All
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" aria-hidden="true" className="transition-transform group-hover:translate-x-0.5">
      ...
      <div className="relative group/tray">
        ...
        <button
          ...
          className={cn(
            "absolute left-2 top-1/2 -translate-y-1/2 z-20 hidden md:flex w-10 h-10 rounded-full bg-black/70 backdrop-blur-sm items-center justify-center text-white transition-all duration-200 hover:bg-black/90 focus-visible:outline-none disabled:pointer-events-none disabled:opacity-0",
            canScrollLeft ? 'opacity-0 group-hover/tray:opacity-100' : 'opacity-0'
          )}
        ...
        <button
          ...
          className={cn(
            "absolute right-2 top-1/2 -translate-y-1/2 z-20 hidden md:flex w-10 h-10 rounded-full bg-black/70 backdrop-blur-sm items-center justify-center text-white transition-all duration-200 hover:bg-black/90 focus-visible:outline-none disabled:pointer-events-none disabled:opacity-0",
            canScrollRight ? 'opacity-0 group-hover/tray:opacity-100' : 'opacity-0'
          )}
        ...
        <div
          ref={scrollRef}
          className="flex gap-3 overflow-x-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden scroll-smooth px-4 sm:px-6 lg:px-20 pt-1 pb-1"
          tabIndex={0}
          ...
          {loading || !titles ? (
            ...
            titles.map((title, i) =>
              numbered ? (
                ...
                <TitleCard key={`${title.mediaType}-${title.id}-${i}`} title={title} index={i} />
              )
            )
          )}
        </div>
