...
function TitleCardComponent({
  ...
  const handleWatchlistClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      toast.success(
        isInWatchlist ? 'Removed from My List' : 'Added to My List',
        { duration: 2000 }
      );
    },
  ...
  return (
    <Link href={`/title/${mediaType}/${id}`}>
      <div
        className="flex-shrink-0 group/card relative w-[130px] md:w-[160px] lg:w-[185px] cursor-pointer"
          ...
          >
            ...
            {progress !== undefined && progress > 0 && (
              <div className="absolute bottom-0 left-0 right-0 h-1 bg-white/20 overflow-hidden">
                <div
                  className="h-full bg-[#4752c4] transition-all duration-500"
          ...
          <div className="flex items-center mt-1 text-[11px] font-medium text-white/50">
            {voteAverage !== undefined && voteAverage > 0 && (
              <span className="flex items-center">
                <svg width="10" height="10" viewBox="0 0 24 24" fill="white" className="mr-1">
      ...
      >
        ...
        <div className="relative flex flex-col w-[110px] sm:w-[130px] lg:w-[160px] z-20 shrink-0">
          ...
          >
            ...
            {imageError && (
              <div className="absolute inset-0 bg-[#1a1c24] flex items-center justify-center">
                <span className="text-white/40 text-xs text-center px-2">{title.title}</span>
              </div>
            )}
          </div>
        </div>
